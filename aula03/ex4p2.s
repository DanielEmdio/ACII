...
main:
    ...
    ori $t1,$t1,0x0001 #define os ports de input com 1, neste caso garantimos que o primeiro switch tem 1 
    sw $t1,TRISB($s0) #define o port de input com 1


while:
    lw $t1, PORTB($s0) # lê o que está no port B
    andi $t1,$t1,0x0001 #nos só queremos o primeiro switch então vamos garantir que os outros todos devolvem 0.
    bgtz $t1,left
    srl $t3,$t3,1
    andi $t3,$t3,0xF # só nos interrrensa os primeiros 4 bits
    bgtz $t3,cont
    li $t3,0x8 # 1000
    j cont
left:
